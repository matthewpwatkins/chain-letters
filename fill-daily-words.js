...
for (let i = 0; i < 100; i++) {
  ...
  if (!fs.existsSync(filePath)) {
    const sourceWord = rl.question(`${fileName} - source: `);
    if (!sourceWord) {
      throw new Error(sourceWord);
    }
    const destinationWord = rl.question(`${fileName} - destination: `);
    if (!destinationWord) {
      throw new Error(destinationWord);
    }
    const bestPath = rl.question(`${fileName} - best path ${sourceWord} => ${destinationWord}: `);
